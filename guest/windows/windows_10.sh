...
#   https://github.com/lordmikefin/kvm-gpu-guide/blob/master/guest/windows/windows_10.sh
...
if [[ -n ${SPICE_PORT} ]]; then
	...
	echo " $ spicy --title 'Windows 10' 127.0.0.1 -p ${SPICE_PORT}"
	echo "You could also use 'remote-viewer'"
	echo " $ remote-viewer --title 'Windows 10' spice://127.0.0.1:${SPICE_PORT}"
fi
