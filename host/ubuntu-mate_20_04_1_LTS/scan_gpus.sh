...
#   https://github.com/lordmikefin/kvm-gpu-guide/blob/master/host/ubuntu-mate_20_04_1_LTS/scan_gpus.sh
...
#		echo ""
	...
	if [ ${IS_AMD} == 1 ] ; then
		...
		let COUNT_SELECTIONS=COUNT_SELECTIONS+1
		NVIDIA_DEVICES_SELECTIONS="${NVIDIA_DEVICES_SELECTIONS} #${COUNT_SELECTIONS} $(lspci -nnk -s ${PCI_BUS_VGA} | grep -i subsystem)  Advanced Micro Devices
${PCI_BUS_VGA}
${PCI_BUS_AUDIO}
"
	fi
